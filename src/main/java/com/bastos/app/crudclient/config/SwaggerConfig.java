...
public class SwaggerConfig {
	...
	public Docket greetingApi() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.bastos.app.crudclient.entrypoint.controller")).paths(PathSelectors.any()).build()
	...
	private ApiInfo apinfo() {
		return new ApiInfoBuilder().title("API - CRUD")
				.description("\"SPRING BOOT REST API - CRUD CLIENT\"")
	...
	private SecurityContext securityContext() {
		return SecurityContext.builder().securityReferences(defaultAuth()).forPaths(PathSelectors.ant(UrlConstants.URL_BASE))
				.build();
	}
